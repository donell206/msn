...
class ControllerMessage extends Controller {
    ...
    public function index() {
        ...
        (new View("messages"))->show(array("recipient" => $recipient, "user" => $user, "messages" => $messages, "messages_json" => $messages_json, "errors" => $errors));
    }

    public function get_visible_messages_service(){
        $user = $this->get_user_or_redirect();
        $recipient = $this->get_recipient($user);
        $messages_json = $recipient->get_visible_messages_as_json($user);
        echo $messages_json;
    ...
    public function add_service(){
        sleep(1); //pour ralentir le service et constater l'effet en AJAX
        $user = $this->get_user_or_redirect();
        $recipient = $this->get_recipient($user);
        echo $this->post($user, $recipient) ? "true" : "false";
    }
